...
def get_available_checkpoints():
    ...
    checkpoints = []
    checkpoint_files = glob.glob(os.path.join("models", "test_1", "checkpoints", "*.pth"))
...
def generate_answer(prompt, image, parameters, checkpoint_name=None):
    ...
    if checkpoint_name:
        checkpoint_path = os.path.join("models", "test_1", "checkpoints", checkpoint_name)
    ...
    return f"Test_1 model response for prompt: {prompt}\nUsing checkpoint: {checkpoint_name}\nParameters: {parameters}" 